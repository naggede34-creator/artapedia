    ...
    <header className="sticky top-0 z-50 border-b border-line bg-bg/90 backdrop-blur">
      <div className="mx-auto flex max-w-content items-center justify-between gap-4 px-5 py-4">
        <Link href="/" className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-md bg-amber text-sm font-bold text-bg">
            A
          </span>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => setOpen((v) => !v)}
            className="hidden items-center gap-2 rounded-full border border-line bg-surface px-3 py-1.5 text-sm text-ink sm:flex"
          >
        ...
        <div className="border-t border-line bg-surface px-5 py-4">
          <div className="mx-auto max-w-content">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <code className="rounded border border-line bg-bg px-3 py-1.5 font-mono text-sm text-teal-bright">
                {token || "..."}
              </code>
      ...
      {menuOpen && (
        <div className="border-t border-line bg-surface px-5 py-4 md:hidden">
          <div className="flex flex-col gap-3">
            ...
            <button
              onClick={() => setOpen((v) => !v)}
              className="mt-1 flex items-center gap-2 rounded-full border border-line bg-bg px-3 py-1.5 text-sm text-ink w-fit"
            >
              <span className="h-1.5 w-1.5 rounded-full bg-teal" />
              {ready ? `Rp${balance.toLocaleString("id-ID")}` : "Memuat..."}
            </button>
